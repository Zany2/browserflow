...
// ClientCheckReq client connection check request 客户端连接检查请求
type ClientCheckReq struct {
	g.Meta   `path:"/check" method:"get" tags:"客户端" summary:"检查客户端连接"`
	ClientID string `json:"client_id,omitempty" in:"query" dc:"客户端 ID"`
}

// ClientCheckRes client connection check response 客户端连接检查响应
...
// ClientOfflineReq force offline request 强制下线请求
...
// ClientOfflineRes force offline response 强制下线响应
type ClientOfflineRes struct {
	Client  *ClientResModel `json:"client,omitempty" dc:"客户端详情"`
	Message string          `json:"message,omitempty" dc:"下线结果消息"`
}

// ClientBatchActionReq batch client action request 批量客户端操作请求
type ClientBatchActionReq struct {
	IDs    []string `json:"ids" v:"required|min-length:1#客户端列表不能为空|至少选择一个客户端" dc:"客户端 ID 列表"`
	Reason string   `json:"reason,omitempty" dc:"操作原因"`
}

// ClientBatchOfflineReq batch force offline request 批量强制下线请求
type ClientBatchOfflineReq struct {
	g.Meta `path:"/batch-offline" method:"post" tags:"客户端" summary:"批量强制客户端下线"`
	ClientBatchActionReq
}

// ClientBatchActionRes batch client action response 批量客户端操作响应
...
// ClientBatchOfflineRes batch force offline response 批量强制下线响应
type ClientBatchOfflineRes struct {
	ClientBatchActionRes
}

// ClientBanReq ban client request 拉黑客户端请求
...
// ClientUnbanRes unban client response 解除拉黑响应
