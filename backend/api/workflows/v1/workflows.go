...
type WorkflowBatchDeleteReq struct {
	...
	IDs    []string `json:"ids" v:"required|min-length:1#工作流id列表不能为空|至少选择一个工作流" dc:"工作流 ID 列表"`
}
...
type WorkflowRunReq struct {
	...
	ID         string                             `json:"id" in:"path" v:"required#工作流ID不能为空" dc:"工作流 ID"`
	BrowserID  string                             `json:"browser_id" dc:"浏览器实例 ID"`
...
type WorkflowExecutionDetailReq struct {
	...
	ExecutionID string `json:"execution_id" in:"path" v:"required#执行ID不能为空" dc:"执行 ID"`
}
