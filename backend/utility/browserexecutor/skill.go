...
// SkillBundleFile is one generated file in the exported browser executor skill bundle.
type SkillBundleFile struct {
	Path    string
	Content string
}

// GenerateSkillBundle builds a multi-file skill bundle for direct browser control.
func GenerateSkillBundle(baseURL string) []SkillBundleFile {
	return []SkillBundleFile{
		{Path: "browserflow-browser-executor/SKILL.md", Content: GenerateSkillIndex(baseURL)},
		{Path: "browserflow-browser-executor/references/api-reference.md", Content: GenerateSkillAPIReference()},
		{Path: "browserflow-browser-executor/references/command-examples.md", Content: GenerateSkillCommandExamples(baseURL)},
		{Path: "browserflow-browser-executor/references/troubleshooting.md", Content: GenerateSkillTroubleshooting()},
	}
}

// GenerateSkillIndex builds the lightweight SKILL.md entry file.
func GenerateSkillIndex(baseURL string) string {
	...
	sb.WriteString("- `references/api-reference.md`: complete endpoint parameters, enums, and shared conventions. Read when command parameters are uncertain.\n")
	sb.WriteString("- `references/command-examples.md`: curl examples for preflight and common browser operations. Read when composing raw HTTP calls.\n")
	sb.WriteString("- `references/troubleshooting.md`: safety boundaries, recovery rules, element identification, response format, and notes. Read when an operation fails or the task is sensitive.\n\n")
	...
	sb.WriteString("1. Use lightweight preflight: check `/app/runtime` and `/browser-executor/status` before the first browser operation in a conversation, before long or sensitive tasks, and after any executor failure. Reuse a recent successful status check for short follow-up operations on the same browser.\n")
	sb.WriteString("2. Treat preflight as a hard gate. If the backend is unreachable, the response `code` is not successful, `status.running` is false, or no BrowserFlow browser-agent page is connected, stop immediately and tell the user the exact readable `message` or `error` instead of attempting a browser action.\n")
	sb.WriteString("3. Before running a task, inspect the current page and create a concrete step plan. Do not start clicking, typing, navigating, or evaluating JavaScript before the plan exists.\n")
	sb.WriteString("4. If only a `#/browser-agent` tab exists, use `tabs` with `action:\"new\"` or call `navigate`; the backend will create a business tab instead of navigating the agent tab.\n")
	sb.WriteString("5. Inspect only the page area or element set needed for the current step. Prefer `input-elements`, `clickable-elements`, `page-structure`, `element-info`, or low-limit `observe` before broad page text or HTML.\n")
	sb.WriteString("6. After navigation or a page-changing action, call `observe` or `snapshot` again.\n")
	sb.WriteString("7. Use RefIDs such as `@e1` from `snapshot` for click, type, get-text, and get-value.\n")
	sb.WriteString("8. Prefer `act` for simple intent-driven actions and set `return_observe:true` after page-changing actions.\n")
	sb.WriteString("9. Prefer `fill-form` for multiple fields and `batch` for deterministic sequential actions; do not batch steps that need observation between them.\n")
	sb.WriteString("10. If a RefID fails, call `observe` or `snapshot` again because the page may have changed.\n")
	sb.WriteString("11. Prefer `page-structure` for structured extraction before requesting full `page-content`, and use `element-info` to diagnose one uncertain element.\n")
	sb.WriteString("12. Use `wait` states precisely: `load`, `visible`, `hidden`, `enabled`, `interactable`, `writable`, `stable`, `dom-stable`, `request-idle`, `elements-more-than`, or `time`. For navigation, set `wait_until` to `load`, `dom-stable`, `request-idle`, `page-stable`, or `none`.\n")
	sb.WriteString("13. Use `cookies` and `storage` when the task needs to inspect login/session state or clean up page state. Prefer these semantic APIs over ad hoc JavaScript.\n")
	sb.WriteString("14. Never close the BrowserFlow browser or any `#/browser-agent` client tab. Before using `close-page` or `tabs` with `action:\"close\"`, call `tabs` with `action:\"list\"` and close only task-related business tabs.\n\n")
	...
	sb.WriteString("2. Validate preflight responses: both calls must return a successful response, the runtime must allow browser executor routes, and `/browser-executor/status` must show `running:true`. If any check fails, stop and report the exact reason to the user.\n")
	sb.WriteString("3. Inspect context: call `observe` on the current page. If the current page is `#/browser-agent` or no useful business page exists, open or navigate to the required business page first, then call `observe` again.\n")
	sb.WriteString("4. Build a plan: write a short numbered plan with the intended browser actions, expected page changes, and the data or final state needed for success.\n")
	sb.WriteString("5. Execute one meaningful step at a time. After every navigation, click, submit, form fill, scroll that reveals content, or JavaScript mutation, call `observe`, `snapshot`, `page-structure`, or a targeted getter to verify the result.\n")
	sb.WriteString("6. If verification fails, stop the current action chain, inspect again, revise the plan, and continue from the verified page state. Do not blindly repeat stale RefIDs or selectors.\n")
	sb.WriteString("7. Use `batch` only for deterministic mini-sequences where no observation is needed between operations. Do not batch an entire unknown workflow.\n")
	sb.WriteString("8. Before reporting success, verify the final page state or extracted data. If the task is incomplete, report the exact blocking condition and the last verified state.\n\n")
	...
	sb.WriteString("- A valid preflight requires a reachable backend, successful response code, and `data.status.running:true` from `/browser-executor/status`.\n")
	sb.WriteString("- If preflight fails, do not call `navigate`, `act`, `tabs`, `observe`, or any page operation. Reply to the user with the exact blocker and ask them to start or reconnect the BrowserFlow browser-agent page.\n")
...
// GenerateSkillAPIReference builds the full endpoint reference file.
func GenerateSkillAPIReference() string {
	var sb strings.Builder
	sb.WriteString("# Browser Executor API Reference\n\n")
	sb.WriteString("Complete parameter details, enums, and shared conventions for BrowserFlow Browser Executor APIs.\n\n")
	appendAllEndpointParameterReference(&sb)
	return sb.String()
}

// GenerateSkillCommandExamples builds the command examples reference file.
func GenerateSkillCommandExamples(baseURL string) string {
	var sb strings.Builder
	sb.WriteString("# Browser Executor Command Examples\n\n")
	sb.WriteString("Use these examples when composing raw HTTP calls. Prefer compact observations and verify after page-changing actions.\n\n")
	...
	sb.WriteString("```\n\n")
	sb.WriteString("Both responses must be successful before any browser command runs. If the backend is unreachable, the response `code` is not successful, or `/browser-executor/status` does not show `data.status.running:true`, stop and reply with the readable `message` or `error`.\n\n")
	appendCoreCommandExamples(&sb, baseURL)
	return sb.String()
}

// GenerateSkillTroubleshooting builds the troubleshooting and safety reference file.
func GenerateSkillTroubleshooting() string {
	var sb strings.Builder
	sb.WriteString("# Browser Executor Troubleshooting\n\n")
	sb.WriteString("Read this file when an operation fails, page state is uncertain, or the task is sensitive.\n\n")
	sb.WriteString("## Safety Boundaries\n\n")
	sb.WriteString("Ask the user for explicit confirmation before destructive, irreversible, or externally visible actions, including submitting purchases, orders, payments, account/security changes, deleting data, sending messages or emails, uploading files, clearing cookies/storage, or changing important settings. If the user already gave clear permission for that exact action in the current request, proceed carefully and verify before submitting.\n\n")
	sb.WriteString("Do not use `evaluate` to bypass user confirmation, disable site protections, read unrelated secrets, or mutate sensitive page state when a semantic BrowserFlow API can do the task. Prefer high-level APIs such as `click`, `type`, `fill-form`, `cookies`, and `storage` over custom JavaScript.\n\n")
	...
	for _, command := range Help("") {
		sb.WriteString(fmt.Sprintf("- `%s /browser-executor%s` - %s\n", command.Method, command.Path, command.Description))
	}
	sb.WriteString("\n## Notes\n\n")
...
func appendCoreCommandExamples(sb *strings.Builder, baseURL string) {
	sb.WriteString("## Core Commands\n\n")
	appendCurl(sb, baseURL, "Open URL", "navigate", map[string]any{"url": "https://example.com", "wait_until": "load"})
	appendCurl(sb, baseURL, "Observe Page", "observe", map[string]any{"include_text": false, "text_limit": 8000})
	appendCurl(sb, baseURL, "Get Snapshot", "snapshot", nil)
