...
func Help(command string) []model.BrowserExecutorHelpCommand {
	commands := []model.BrowserExecutorHelpCommand{
		...
		{Name: "navigate", Method: "POST", Path: "/navigate", Description: "Open URL and wait for load/dom-stable/request-idle/page-stable/none", Parameters: []string{"url", "wait_until", "timeout"}, Example: map[string]any{"url": "https://example.com", "wait_until": "load"}},
		...
		{Name: "cookies", Method: "POST", Path: "/cookies", Description: "List, set, delete, or clear browser cookies", Parameters: []string{"action", "name", "value", "url", "domain", "path", "secure", "http_only", "same_site", "expires"}},
		{Name: "storage", Method: "POST", Path: "/storage", Description: "List, get, set, delete, or clear localStorage/sessionStorage on the current page", Parameters: []string{"action", "type", "key", "value"}},
