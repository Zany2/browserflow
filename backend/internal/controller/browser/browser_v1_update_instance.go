...
// BrowserInstanceUpdate updates browser instance 更新浏览器实例
func (c *ControllerV1) BrowserInstanceUpdate(ctx context.Context, req *v1.BrowserInstanceUpdateReq) (res *v1.BrowserInstanceUpdateRes, err error) {
	state.DBMu.Lock()
	if state.DB == nil {
	...
	existing, err := db.GetBrowserInstance(req.ID)
	if err != nil {
		return nil, err
	}
	...
	return &v1.BrowserInstanceUpdateRes{Instance: instance}, nil
}
