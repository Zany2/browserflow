...
func (c *ControllerV1) BrowserStart(ctx context.Context, req *v1.BrowserStartReq) (res *v1.BrowserStartRes, err error) {
	state.DBMu.Lock()
	if state.DB == nil {
	...
	instance, err := db.GetDefaultBrowserInstance()
	if err != nil {
		headless := false
		...
		if saveErr := db.SaveBrowserInstance(instance); saveErr != nil {
			return nil, saveErr
		}
	}
