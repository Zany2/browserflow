...
// LlmConfigUpdate updates model config 更新大模型配置
func (c *ControllerV1) LlmConfigUpdate(ctx context.Context, req *v1.LlmConfigUpdateReq) (res *v1.LlmConfigUpdateRes, err error) {
	state.DBMu.Lock()
	if state.DB == nil {
	...
	existing, err := db.GetLLMConfig(req.ID)
	if err != nil {
		return nil, err
	}
	...
	return &v1.LlmConfigUpdateRes{Config: config}, nil
}
