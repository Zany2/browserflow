...
// AutomaWorkflowSyncCandidates lists local-file sync candidates 获取本地文件同步候选
func (c *ControllerV1) AutomaWorkflowSyncCandidates(ctx context.Context, req *v1.AutomaWorkflowSyncCandidatesReq) (res *v1.AutomaWorkflowSyncCandidatesRes, err error) {
	state.DBMu.Lock()
	if state.DB == nil {
	...
	candidates := make([]v1.AutomaWorkflowSyncCandidatesResModel, 0, len(items))
	for _, item := range items {
		...
		candidate := v1.AutomaWorkflowSyncCandidatesResModel{Id: automaID, AutomaId: automaID, WorkflowId: automaID, Name: name, Description: description, Source: "客户端同步", SourceIp: req.SourceIP, AutomaVersion: strings.TrimSpace(gconv.String(item["version"])), ExtVersion: strings.TrimSpace(gconv.String(item["extVersion"])), CreatedAtAutoma: gconv.Int64(item["createdAt"]), UpdatedAtAutoma: gconv.Int64(item["updatedAt"]), IsDisabled: gconv.Bool(item["isDisabled"]), NodeCount: nodeCount, EdgeCount: edgeCount, ContentHash: contentHash, Synced: synced, HasUpdate: hasUpdate, SyncStatus: status}
		if serverRecord != nil {
	...
	if start >= total {
		return &v1.AutomaWorkflowSyncCandidatesRes{List: []v1.AutomaWorkflowSyncCandidatesResModel{}, Total: total}, nil
	}
	end := start + pageSize
	if end > total {
		end = total
	}
	return &v1.AutomaWorkflowSyncCandidatesRes{List: candidates[start:end], Total: total}, nil
}
