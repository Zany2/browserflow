...
// AutomaWorkflowCreate creates or updates local workflow records 新增或更新本地工作流
func (c *ControllerV1) AutomaWorkflowCreate(ctx context.Context, req *v1.AutomaWorkflowCreateReq) (res *v1.AutomaWorkflowCreateRes, err error) {
	...
	var metas []v1.AutomaWorkflowCreateMeta
	...
	stats := v1.AutomaWorkflowMutationStats{Submitted: len(req.WorkflowFiles)}
	for index, workflowFile := range req.WorkflowFiles {
		...
		meta := v1.AutomaWorkflowCreateMeta{Source: 1}
		if index < len(metas) {
			meta = metas[index]
		}
		if meta.Source == 0 {
	...
	return &v1.AutomaWorkflowCreateRes{AutomaWorkflowMutationStats: stats}, nil
}
