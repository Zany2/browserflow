...
// AutomaWorkflowList returns local-file workflow records 返回本地文件中的工作流列表
func (c *ControllerV1) AutomaWorkflowList(ctx context.Context, req *v1.AutomaWorkflowListReq) (res *v1.AutomaWorkflowListRes, err error) {
	state.DBMu.Lock()
	if state.DB == nil {
	...
	list := make([]v1.AutomaWorkflowListResModel, 0, end-start)
	for _, record := range filtered[start:end] {
		...
		item := v1.AutomaWorkflowListResModel{Id: record.ID, AutomaId: record.AutomaID, Name: record.Name, Description: record.Description, Source: sourceText, SourceIp: record.SourceIP, CreatedAtAutoma: record.CreatedAtAutoma, UpdatedAtAutoma: record.UpdatedAtAutoma, IsDisabled: record.IsDisabled, IsProtected: record.IsProtected, NodeCount: record.NodeCount, EdgeCount: record.EdgeCount, ContentHash: record.ContentHash, Revision: record.Revision}
	...
	return &v1.AutomaWorkflowListRes{List: list, Total: len(filtered)}, nil
}
