...
type Tasks struct {
	...
	ClientIp              interface{} // 目标客户端 IP，保留用于兼容旧版按 IP 调度
	MachineId             interface{} // 目标机器 ID，为空表示不限定机器
	NodeId                interface{} // 目标执行节点 ID，为空表示由服务端选择可用节点
	TargetGroupId         interface{} // 目标节点分组 ID，为 0 表示不限定分组
	DispatchMode          interface{} // 调度模式：auto 自动选择，group 指定分组，machine 指定机器，node 指定节点，ip 兼容旧 IP
