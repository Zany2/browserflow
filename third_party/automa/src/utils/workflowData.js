...
function findIncludedWorkflows(
  ...
  const checkWorkflow = (type, workflowId) => {
    ...
    if (workflow) {
      // BrowserFlow local change start: keep child workflow identity in exports 保留子工作流导出的原始标识
...
export function exportWorkflow(workflow) {
  ...
  // BrowserFlow local change start: keep workflow identity in exported json 保留主工作流导出的原始标识
